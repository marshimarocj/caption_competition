...
if __name__ == '__main__':
  ...
  m = rank_model.aca_parallel.Model(model_cfg)

  if opts.is_train:
    trntst = rank_model.aca_parallel.TrnTst(model_cfg, path_cfg, m)

    trn_reader = rank_model.aca_parallel.TrnReader(
      model_cfg.num_neg, path_cfg.trn_ftfiles, path_cfg.trn_annotation_file)
    val_reader = rank_model.aca_parallel.ValReader(
    ...
    tst_reader = rank_model.aca_parallel.TstReader(opts.ft_files.split(','), opts.annotation_file)
