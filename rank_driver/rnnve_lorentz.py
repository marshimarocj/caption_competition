...
WE = rank_model.rnnve_lorentz.WE
RNN = rank_model.rnnve_lorentz.RNN
CELL = rank_model.rnnve_lorentz.CELL
RCELL = rank_model.rnnve_lorentz.RCELL
...
if __name__ == '__main__':
  ...
  m = rank_model.rnnve_lorentz.Model(model_cfg)

  if opts.is_train:
    trntst = rank_model.rnnve_lorentz.TrnTst(model_cfg, path_cfg, m)

    trn_reader = rank_model.rnnve_lorentz.TrnReader(
      model_cfg.num_neg, path_cfg.trn_ftfiles, path_cfg.trn_annotation_file)
    val_reader = rank_model.rnnve_lorentz.ValReader(
    ...
    tst_reader = rank_model.rnnve_lorentz.TstReader(opts.ft_files.split(','), opts.annotation_file)
