...
WE = rank_model.rnnve_feedforward.WE
RNN = rank_model.rnnve_feedforward.RNN
CELL = rank_model.rnnve_feedforward.CELL
RCELL = rank_model.rnnve_feedforward.RCELL
...
if __name__ == '__main__':
  ...
  m = rank_model.rnnve_feedforward.Model(model_cfg)

  if opts.is_train:
    trntst = rank_model.rnnve_feedforward.TrnTst(model_cfg, path_cfg, m)

    trn_reader = rank_model.rnnve_feedforward.TrnReader(
      model_cfg.num_neg, path_cfg.trn_ftfiles, path_cfg.trn_annotation_file)
    val_reader = rank_model.rnnve_feedforward.ValReader(
    ...
    tst_reader = rank_model.rnnve_feedforward.TstReader(opts.ft_files.split(','), opts.annotation_file)
