...
WE = rank_model.srnnve.WE
RNN = rank_model.srnnve.RNN
CELL = rank_model.srnnve.CELL
...
if __name__ == '__main__':
  ...
  m = rank_model.srnnve.Model(model_cfg)

  if opts.is_train:
    trntst = rank_model.srnnve.TrnTst(model_cfg, path_cfg, m)

    trn_reader = rank_model.srnnve.TrnReader(
      model_cfg.num_neg, path_cfg.trn_ftfiles, path_cfg.trn_annotation_file)
    val_reader = rank_model.srnnve.ValReader(
    ...
    tst_reader = rank_model.srnnve.TstReader(opts.ft_files.split(','), opts.annotation_file)
