...
WE = rank_model.rnnve_boost.WE
RNN = rank_model.rnnve_boost.RNN
CELL = rank_model.rnnve_boost.CELL
RCELL = rank_model.rnnve_boost.RCELL
...
if __name__ == '__main__':
  ...
  m = rank_model.rnnve_boost.Model(model_cfg)

  if opts.is_train:
    if model_cfg.loss == 'orth':
      trntst = rank_model.rnnve_boost.OrthTrnTst(model_cfg, path_cfg, m)
      trn_reader = rank_model.rnnve_boost.OrthReader(
        model_cfg.num_neg, path_cfg.trn_ftfiles, path_cfg.trn_annotation_file)
      val_reader = rank_model.rnnve_boost.OrthReader(
      ...
      trntst = rank_model.rnnve_boost.TrnTst(model_cfg, path_cfg, m)
      trn_reader = rank_model.rnnve_boost.TrnReader(
        model_cfg.num_neg, path_cfg.trn_ftfiles, path_cfg.trn_annotation_file)
      val_reader = rank_model.rnnve_boost.ValReader(
    ...
    tst_reader = rank_model.rnnve_boost.TstReader(opts.ft_files.split(','), opts.annotation_file)
