...
import rank_model.rnnve_output
import common

WE = rank_model.rnnve_output.WE
RNN = rank_model.rnnve_output.RNN
CELL = rank_model.rnnve_output.CELL
RCELL = rank_model.rnnve_output.RCELL
...
if __name__ == '__main__':
  ...
  m = rank_model.rnnve_output.Model(model_cfg)

  path_cfg.model_file = os.path.join(path_cfg.model_dir, 'epoch-%d'%opts.best_epoch)
  path_cfg.predict_file = os.path.join(path_cfg.output_dir, 'pred', '%s.rnn.npz'%opts.out_name)
  path_cfg.log_file = None

  trntst = rank_model.rnnve_output.TrnTst(model_cfg, path_cfg, m)

  tst_reader = rank_model.rnnve_output.TstReader(opts.annotation_file)
