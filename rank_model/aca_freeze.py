...
class Model(framework.model.module.AbstractModel):
  ...
  def _add_input_in_mode(self, mode):
    with tf.variable_scope(self.name_scope):
      ...
      captions = tf.placeholder(
        tf.float32, shape=(None, self._config.max_words_in_caption, self._config.dim_caption), name='captions')
  ...
  def _build_parameter_graph(self):
    with tf.variable_scope(self.name_scope):
      ...
      self.caption_pca_W = tf.contrib.framework.model_variable('caption_pca_W',
        shape=(2*self._config.dim_caption, self._config.dim_joint_embed), dtype=tf.float32,
        initializer=tf.contrib.layers.xavier_initializer())
  ...
  def get_out_ops_in_mode(self, in_ops, mode, **kwargs):
    wvecs = in_ops[self.InKey.CAPTIONID]
