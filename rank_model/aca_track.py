  ...
  def get_out_ops_in_mode(self, in_ops, mode, **kwargs):
    ...
    def trn(wvecs, fts, word_mask, ft_mask, is_trn):
      with tf.variable_scope(self.name_scope):
        ...
        def calc_neg_word_sim(pos_fts, neg_wvecs, neg_alpha, pos_beta, neg_word_mask, pos_ft_mask):
          ...
          # aggregate
          neg_sim = tf.reduce_sum(wvec_compare * tf.expand_dims(neg_word_mask, 2), 1) / tf.reduce_sum(neg_word_mask, 1, True)
          neg_sim += tf.reduce_sum(ft_compare * tf.expand_dims(pos_ft_mask, 0), 2) / tf.reduce_sum(pos_ft_mask, 1, True)
          neg_sim /= 2.
        ...
        def calc_neg_ft_sim(neg_fts, pos_wvecs, pos_alpha, neg_beta, pos_word_mask, neg_ft_mask):
          ...
          neg_sim += tf.reduce_sum(ft_compare * tf.expand_dims(neg_ft_mask, 2), 1) / tf.reduce_sum(neg_ft_mask, 1, True)
          neg_sim /= 2.
    ...
    def tst(wvecs, fts, word_mask, ft_mask, is_trn):
      with tf.variable_scope(self.name_scope):
        ...
        beta = tf.nn.xw_plus_b(tf.reshape(fts, (-1, dim_ft)), self.ft_att_W, self.ft_att_B)
