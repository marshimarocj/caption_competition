...
if __name__ == '__main__':
  ...
  if opts.is_train:
    m = gen_model.diversity.Model(model_cfg)
    ...
    trntst = gen_model.diversity.TrnTst(model_cfg, path_cfg, m)

    trn_reader = gen_model.diversity.Reader(
      path_cfg.trn_ftfiles, path_cfg.trn_videoid_file,
      shuffle=True, annotation_file=path_cfg.trn_annotation_file)
    val_reader = gen_model.diversity.Reader(
    ...
    model_cfg.tst_strategy = opts.tst_strategy
    model_cfg.tst_num_sample = opts.tst_num_sample
    model_cfg.tst_sample_topk = opts.tst_sample_topk

    m = gen_model.diversity.Model(model_cfg)

    trntst = gen_model.diversity.TrnTst(model_cfg, path_cfg, m, gen_sent_mode=opts.gen_sent_mode)

    tst_reader = gen_model.diversity.Reader(
      path_cfg.tst_ftfiles, path_cfg.tst_videoid_file, shuffle=False)
